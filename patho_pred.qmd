...
condition_patho <- quo(patho == "1" & row_number() == min(which(patho == "1"))) # Sample on the first day of symptoms
condition_first <- quo(day == min(day) & pathology_during == "1") # First sample of subject with later infection
condition_pre <- quo(lead(patho == "1" & row_number() == min(which(patho == "1")))) # Sample on the day preceding symptoms
condition_last <- quo(day == max(day) & pathology_during != "1") # Samples of the last camp day of healthy athletes

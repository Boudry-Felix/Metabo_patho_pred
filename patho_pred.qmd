...
categories <- c("patho", "first", "rand", "pre")
data <- fread(input = "Data/data.csv") |>
...
data_first <- filter(data, data$first == "1" | data$patho == "1")
data_pre <- filter(data, data$pre == "1" | data$patho == "1")
data_rand <- filter(data, data$rand == "1" | data$patho == "1")

data_sub <- lst(data_first, data_pre, data_rand)
...
lapply(data_sub, \(x) {
  ...
  fviz_cluster(
    ...
    geom_point(
      mapping = aes(shape = x$patho |> as.factor()),
      size = 2
    ) +
    labs(shape = "Status")
...
# DisciMiner::plsDA(select(.data = data_rand, -any_of(categories)), as.factor(x = data_rand$Patho), cv = "LOO")
# KODAMA::pls.double.cv(select(.data = data_pre_post, -any_of(categories)), as.factor(x = data_pre_post$Patho))
```

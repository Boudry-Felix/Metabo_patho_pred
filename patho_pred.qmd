...
data_rand <- fread(input = "Data/data_Patho_rand.csv") |> 
  column_to_rownames(var = "Samplecode") |> 
  mutate_at(c(2:280), \(x) ReplaceMissingByLoD(x)) |>
  mutate_at(c(2:280), \(x) scale(x)) |> 
  lapply(as.numeric) |> 
  as.data.frame()
```
...
```{r PLS-DA_PP}
pls_res <- caret::plsda(data_pre_post, as.factor(data_pre_post$Patho))
ggplot(pls_res$model, aes(pls_res$scores[, 1], pls_res$scores[, 2])) +
  geom_point()

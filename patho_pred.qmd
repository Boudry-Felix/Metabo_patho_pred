...
lapply(data_sub, \(x) {
  ...
  # Confusion matrix
  km_cluster <- km_res$cluster - 1
  conf_mat_plot <- confusion_matrix(targets = x$filter_condition, predictions = km_cluster) |>
    plot_confusion_matrix()
  conf_mat <- confusionMatrix(data = as.factor(x = km_cluster), as.factor(x = x$filter_condition))
...
lapply(data_sub, \(x) {
  ...
  # Validation metrics
  valid_metrics <- pls.double.cv(select(.data = x, -any_of(categories)), as.factor(x = x$filter_condition), scaling = NULL, times = 10, runn = 10)[c("R2Y", "Q2Y")]

...
Pre-processing was done using [npflow](https://nmrprocflow.org). This pre-processing is based on the [Rnmr1D](https://cran.r-project.org/web/packages/Rnmr1D/index.html) package and the macro file can be found in the npflow folder.
...
data <- fread("Data/data_clean.csv") |> 
  ...
  mutate(across(any_of(categories), as.factor)) |> 
  merge(fread("Data/samples_clean.csv"), by = "Samplecode") |> 
  replace_na(list(patho = 0)) |>
  replace_na(list(pathology_during = 0)) |>  
  column_to_rownames(var = "Samplecode") |> 
  mutate(patho = as.factor(patho))

# Define subsets with simplified filters
data_first_patho <- data |> 
  group_by(name, surname) |> 
  arrange(day) |> 
  filter(day == min(day) & pathology_during == "1" | patho == "1" & row_number() == min(which(patho == "1"))) |> 
  suppressWarnings() |> 
  ungroup() |> 
  as.data.frame()
...
data_pre_patho <- data |> 
  group_by(name, surname) |> 
  arrange(day) |> 
  filter(lead(patho == "1" & row_number() == min(which(patho == "1"))) | patho == "1" & row_number() == min(which(patho == "1"))) |> 
  suppressWarnings() |> 
  ungroup() |> 
  as.data.frame()
...
data_last_patho <- data |> 
  group_by(name, surname) |> 
  arrange(day) |> 
  filter(day == max(day) & pathology_during != "1" | patho == "1" & row_number() == min(which(patho == "1"))) |> 
  suppressWarnings() |> 
  ungroup() |> 
  as.data.frame()
...
lapply(data_sub, \(x){
  KODAMA::pls.double.cv(select(.data = x, -any_of(categories)), as.factor(x = x$patho))$results[[2]][c("R2Y", "Q2Y")]
})

lapply(data_sub, \(x){
  DiscriMiner::plsDA(select(.data = x, -any_of(categories)), as.factor(x = x$patho), cv = "LOO")[c("R2", "Q2")]
})
```
...
#| label: corr
#| fig-align: center
spectra <- fread("Data/data_clean.csv") |> 
